...
```bash
git clone -b hcita/human_detect git@fp-Gitlab.fitipower.inc:hcita/yolov5.git  # clone
cd yolov5
...
Inference within `HUMAN_DET_1.002.001`(Default) , you can run the command below:

```bash
bash testing_data_inference.sh
```

runs inference on a variety of sources, downloading [models](https://fp-gitlab/hcita/yolov5/-/blob/hcita/human_detect/runs/train/yolov5n_WM009_DM033_pretrain_96x1_b64_ISP_mix_0803_data_config_2023-08-07_5000epc_NC1_no_mosaic/weights/best-int8.tflite) automatically and saving results to `output/`.



We can modify the following parameters for `testing_data_inference.sh`

```python

SOURCE = "" (Path of the folder and the name of the folder should be "human" if the class is 0 (human))
WEIGHTS = "" (Tflite model only)
I = 96,320,640... (Image Resolution)
...
The commands below reproduce YOLOv5 [HUMAN_DET_1.002.001(training data: user can modify/add images for this dataset)](https://fp-gitlab/hcita/yolov5/-/tree/hcita/human_detect/datasets/images/For_training/images/isp) and its [labels](https://fp-gitlab/hcita/yolov5/-/tree/hcita/human_detect/datasets/images/For_training/labels/isp)

Training within `HUMAN_DET_1.002.001`(Default) , you can run the command below:

```bash
bash training_model.sh
```

We can modify the following parameters for bash `training_model.sh`

```python
D = "" (Data config path)
C = "" (Model config path)
H = "" (Hyper Parameters config path)
WEIGHTS = "" (Pretrain Model)
I = 96,320,640... (Image Resolution)
B = 64,128,... (Batch Size)
E = 100,300,1000... (Epochs)

```

<img width="600" src="https://fp-gitlab/hcita/yolov5/-/blob/hcita/human_detect/HUMAN_DET_1.002.001.png">
...
```bash
bash export_int8_tflite.sh
```
We can modify the following parameters for bash `bash export_int8_tflite.sh`

```python
D = "" (Data for cabli)
WEIGHTS = "" (The Path of Pytorch Model)
I = 96,320,640... (Image Resolution)
```
